...
//   // Orientation and dimensions state
...
//   // Updated category render with landscape responsive spacing
//   const renderCategory = useCallback(({ item }) => (
//     <TouchableOpacity
//       style={[
//         styles.categoryCardContainer,
//         isLandscape && {
//           marginRight: (screenWidth - (86 * 4)) / 5 // Distribute remaining space evenly
//         }
//       ]}
...
//       </View>
//     </TouchableOpacity>
//   ), [navigation, isLandscape, screenWidth]);
...
//         {/* Categories - Now responsive in landscape */}
//         <View style={[
//           styles.categoriesContainer,
//           isLandscape && {
//             maxWidth: '100%',
//             paddingHorizontal: 20 // Better padding in landscape
//           }
//         ]}>
//           <FlatList
//             data={categories}
...
//             scrollEnabled={!isLandscape} // Disable scroll in landscape since all 4 fit
//           />
//         </View>
...
const Home = ({ navigation }) => {
  ...
  const onSlideChange = useCallback(
    event => {
      const contentWidth = isTablet ? screenWidth - 64 : screenWidth - 32;
  ...
  const renderBannerItem = useCallback(
    ({ item }) => (
      <View
        style={[
          isTablet ? tabletStyles.bannerSlide : styles.bannerSlide,
          { width: isTablet ? screenWidth - 64 : screenWidth - 32 },
        ]}
      >
        <Image
          source={item.image}
          style={isTablet ? tabletStyles.bannerImage : styles.bannerImage}
          resizeMode="contain"
        />
      </View>
    ),
  ...
  const renderCounterButton = useCallback(
    product => {
      const count = getItemQuantity(product.id);
      const buttonStyles = isTablet ? tabletStyles : styles;
      ...
      if (count === 0) {
        ...
        return (
          <View style={buttonStyles.counterContainer}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Delete width={isTablet ? 30 : 15} height={isTablet ? 30 : 15} />
            </TouchableOpacity>
            <Text style={buttonStyles.counterText}>{count}</Text>
            <TouchableOpacity
        ...
        return (
          <View style={buttonStyles.counterContainer}>
            <TouchableOpacity
              style={buttonStyles.decrementButton}
              onPress={() => handleDecrement(product.id)}
              activeOpacity={0.7}
            >
              <Minusicon
  ...
  // Updated category render with tablet support
  const renderCategory = useCallback(
    ({ item }) => {
      ...
      return (
        <TouchableOpacity
          style={categoryStyles.categoryCardContainer}
          onPress={() => navigation.navigate(item.screen)}
          activeOpacity={0.8}
        >
          <Image source={item.img} style={categoryStyles.categoryImage} />
          <View style={categoryStyles.categoryCard}>
            <Text style={categoryStyles.categoryText}>{item.title}</Text>
          </View>
        </TouchableOpacity>
      );
    },
  ...
  // New render function for New Arrival products with tablet support
  const renderNewArrivalProduct = useCallback(
    ({ item }) => {
      const productStyles = isTablet ? tabletStyles : styles;

      return (
        <View style={productStyles.newArrivalProductCard}>
          <TouchableOpacity
            onPress={() =>
          ...
          <View style={productStyles.productTags}>
            <Text style={productStyles.productTag}>{item.category}</Text>
            <Text style={productStyles.productTag}>{item.subcategory}</Text>
          </View>
          <Text style={productStyles.packSize}>
            Pack Size:{' '}
            <Text style={productStyles.packSizeValue}>{item.packSize}</Text>
          </Text>
    ...
    [navigation, renderCounterButton, isTablet],
  );

  const renderLiveOffer = useCallback(
    ({ item }) => {
      const offerStyles = isTablet ? tabletStyles : styles;

      return (
        <View style={offerStyles.liveOfferProductCard}>
          <View style={offerStyles.offerBadge}>
            <Text style={offerStyles.offerBadgeText}>{item.offerText}</Text>
          </View>
          <TouchableOpacity
            onPress={() =>
              navigation.navigate('SingleProduct', { product: item })
            ...
            activeOpacity={0.8}
          >
            <View style={offerStyles.productImageContainer}>
              <Image
                source={item.image}
                style={offerStyles.productImage}
                resizeMode="cover"
              />
            </View>
          </TouchableOpacity>
          <Text style={offerStyles.productName}>{item.name}</Text>
          <View style={offerStyles.productTags}>
            <Text style={offerStyles.productTag}>{item.category}</Text>
            <Text style={offerStyles.productTag}>{item.subcategory}</Text>
          </View>
          <Text style={offerStyles.packSize}>
            Pack Size:{' '}
            <Text style={offerStyles.packSizeValue}>{item.packSize}</Text>
          </Text>
          <View style={offerStyles.priceContainer}>
            <Text style={offerStyles.wasText}>
              Was:{' '}
              <Text style={offerStyles.originalPrice}>
                {item.originalPrice}
              </Text>
            </Text>
            <Text style={offerStyles.nowText}>Now:</Text>
          </View>
          <View style={offerStyles.productFooter}>
            <Text style={offerStyles.productPrice}>{item.currentPrice}</Text>
            {renderCounterButton(item)}
          </View>
        </View>
      );
  ...
  const renderProductCategory = useCallback(
    ({ item }) => {
      const catStyles = isTablet ? tabletStyles : styles;

      return (
        <TouchableOpacity
          key={item.id}
          style={catStyles.productCategoryItem}
          activeOpacity={0.8}
        >
          <View style={catStyles.productCategoryImageContainer}>
            <Image
              source={{ uri: item.image }}
              style={catStyles.productCategoryImage}
            />
          </View>
          <Text style={catStyles.productCategoryName}>{item.name}</Text>
          <Text style={catStyles.productCategoryCount}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <LinearGradient
        ...
        style={isTablet ? tabletStyles.header : styles.header}
      >
        <View
          style={isTablet ? tabletStyles.logoContainer : styles.logoContainer}
        >
          <UnitedFoodLogo />
        </View>
        ...
        <View
          style={
            isTablet ? tabletStyles.searchContainer : styles.searchContainer
          }
        >
          <TouchableOpacity
            style={isTablet ? tabletStyles.searchBar : styles.searchBar}
            onPress={() => navigation.navigate('Search')}
            activeOpacity={0.7}
          >
            <Text
              style={
                isTablet
                  ? tabletStyles.searchPlaceholder
          ...
          <TouchableOpacity
            ...
            style={
              isTablet
                ? tabletStyles.notificationButton
          ...
          >
            <Image
              ...
              style={
                isTablet
                  ? tabletStyles.logoNotification
                  : styles.logoNotification
              }
            />
          </TouchableOpacity>
        </View>
      ...
      <ScrollView
        style={styles.scrollView}
        contentContainerStyle={
          isTablet ? tabletStyles.scrollContent : styles.scrollContent
        }
        showsVerticalScrollIndicator={false}
        bounces={true}
        bouncesZoom={false}
      ...
      >
        ...
        <View
          style={
            isTablet ? tabletStyles.bannerContainer : styles.bannerContainer
          }
        >
          <FlatList
            ref={sliderRef}
            ...
            onMomentumScrollEnd={onSlideChange}
          />
          <View style={isTablet ? tabletStyles.bannerDots : styles.bannerDots}>
            {bannerImages.map((item, index) => (
              <View
                key={`banner-dot-${item.id}-${index}`}
                style={[
                  isTablet ? tabletStyles.dot : styles.dot,
                  currentSlide === index &&
                    (isTablet ? tabletStyles.activeDot : styles.activeDot),
                ]}
              />
            ))}
          </View>
        ...
        {/* Categories - Tablet optimized to cover full screen */}
        <View
          style={
            isTablet
              ? tabletStyles.categoriesContainer
              : styles.categoriesContainer
          }
        >
          <FlatList
            ...
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={
              isTablet ? tabletStyles.categoriesGrid : styles.categoriesGrid
            }
            removeClippedSubviews={Platform.OS === 'android'}
            maxToRenderPerBatch={4}
            windowSize={5}
        ...
        {/* New Arrival Section */}
        <View
          style={isTablet ? tabletStyles.sectionHeader : styles.sectionHeader}
        >
          <Text
            style={isTablet ? tabletStyles.sectionTitle : styles.sectionTitle}
          >
            New Arrival
          </Text>
          <TouchableOpacity
            style={
              isTablet ? tabletStyles.viewAllContainer : styles.viewAllContainer
            }
            onPress={() => navigation.navigate('Browse')}
            activeOpacity={0.7}
          >
            <Text
              style={isTablet ? tabletStyles.viewAllText : styles.viewAllText}
            >
              View All
            </Text>
            <Image
              source={arrow}
              style={isTablet ? tabletStyles.arrowImage : styles.arrowImage}
            />
          </TouchableOpacity>
        </View>
        ...
        <View
          style={
            isTablet
              ? tabletStyles.newArrivalContainer
              : styles.newArrivalContainer
          }
        >
          <FlatList
            data={newArrivalProducts}
            ...
            contentContainerStyle={
              isTablet
                ? tabletStyles.newArrivalScrollContent
                : styles.newArrivalScrollContent
            }
            removeClippedSubviews={Platform.OS === 'android'}
            maxToRenderPerBatch={4}
            windowSize={5}
        ...
        {/* Product Categories Section - Tablet: Horizontal Scroll in One Row */}
        <View style={isTablet ? tabletStyles.newSection : styles.newSection}>
          <View
            style={isTablet ? tabletStyles.sectionHeader : styles.sectionHeader}
          >
            <Text
          ...
          {isTablet ? (
            ...
            <FlatList
              ...
              keyExtractor={item => `product-category-${item.id}`}
              horizontal
              showsHorizontalScrollIndicator={false}
              contentContainerStyle={tabletStyles.productCategoryScrollContent}
              removeClippedSubviews={Platform.OS === 'android'}
            />
          ) : (
            // Mobile: Grid layout
        ...
        <View style={isTablet ? tabletStyles.newSection : styles.newSection}>
          ...
          <View
            style={
              isTablet
                ? tabletStyles.liveOffersContainer
                : styles.liveOffersContainer
            }
          >
            <FlatList
              data={liveOffers}
              ...
              contentContainerStyle={
                isTablet
                  ? tabletStyles.liveOffersScrollContent
                  : styles.liveOffersScrollContent
              }
              removeClippedSubviews={Platform.OS === 'android'}
              maxToRenderPerBatch={4}
              windowSize={5}
...
const tabletStyles = StyleSheet.create({
  ...
  // Categories - Full screen width for tablet
  categoriesContainer: {
    paddingHorizontal: 30,
    marginTop: 20,
    marginBottom: 10,
  ...
  // New Arrival - Increased height for tablet
  newArrivalContainer: {
    marginBottom: 25,
  },
  newArrivalScrollContent: {
  ...
  // Live Offers - Increased height for tablet
  liveOffersContainer: {
    marginBottom: 25,
  },
  liveOffersScrollContent: {
  ...
  // Product card common styles for tablet
  productImageContainer: {
    alignItems: 'center',
    marginBottom: 15,
  },
  ...
  // Product Categories - Horizontal scroll for tablet
  newSection: {
    marginTop: 30,
    paddingHorizontal: 0,
  },
  productCategoryScrollContent: {
    paddingLeft: 30,
    paddingRight: 30,
  },
  productCategoryItem: {
