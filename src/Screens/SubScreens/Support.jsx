...
You can also save a new payment option directly while checking out – just tick Save this payment method and it will be added to your Wallet.`
  ...
  const iconSize = isTablet ? 36 : 25;

  const renderFAQItem = (item) => (
    <View key={item.id} style={currentStyles.faqItem}>
      <TouchableOpacity 
        style={currentStyles.faqHeader}
        onPress={() => toggleFAQ(item.id)}
      >
        <Text style={currentStyles.faqQuestion}>{item.question}</Text>
        <Text style={currentStyles.expandIcon}>
          {expandedFAQ === item.id ? '×' : '+'}
        </Text>
      </TouchableOpacity>
      
      {expandedFAQ === item.id && (
        <View style={currentStyles.faqAnswer}>
          <Text style={currentStyles.answerText}>{item.answer}</Text>
        </View>
      )}
    </View>
  );
  ...
  return (
    <ScrollView style={currentStyles.container} showsVerticalScrollIndicator={false}>
      <View style={currentStyles.section}>
        <Text style={currentStyles.sectionTitle}>FAQs</Text>
        <View style={currentStyles.faqContainer}>
          {faqData.map(renderFAQItem)}
        </View>
      </View>

      <View style={currentStyles.section}>
        <Text style={currentStyles.sectionTitle}>Customer Support</Text>
        <TouchableOpacity style={currentStyles.supportOption}>
          <View style={currentStyles.supportContent}>
            <Hwhatsapp
              width={iconSize}
              height={iconSize}
              fill="#648F00"
            ...
            />
            <Text style={currentStyles.supportText}>Chat on WhatsApp</Text>
          </View>
      ...
      <View style={currentStyles.section}>
        <Text style={currentStyles.sectionTitle}>Terms & Conditions</Text>
        <TouchableOpacity style={currentStyles.supportOption}>
          <View style={currentStyles.supportContent}>
            <Termscondition
              width={iconSize}
              ...
              style={{ marginRight: isTablet ? 18 : 13 }}
            />
            <Text style={currentStyles.supportText}>Terms & Conditions</Text>
          </View>
          <Text style={currentStyles.arrow}>›</Text>
        </TouchableOpacity>
      </View>
